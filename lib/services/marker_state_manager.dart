...
class MarkerStateManager {
  ...
  Future<BitmapDescriptor> _createMarker({
    ...
    try {
      ...
      _enqueueRetry(cacheKey, userId, photoUrl, displayName, isPaused);

      // Retornar fallback inmediatamente
      return _createFallbackMarker();
    }
  }
  ...
  Future<BitmapDescriptor> _buildMarkerIcon({
    ...
    try {
      ...
      // Fondo circular teal (o gris si está pausado)
      final paint = Paint()
        ..color = isPaused
  ...
  /// Crea marcador fallback (teal) cuando falla carga de foto
  BitmapDescriptor _createFallbackMarker() {
